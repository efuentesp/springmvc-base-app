...
import com.softtek.acceleo.demo.wizard.repository.BeneficiarioRepository;
...
public class BeneficiarioRepositoryImpl implements BeneficiarioRepository {
	...
	public List<Beneficiario> listBeneficiariosQuery(Beneficiario beneficiario, String query, int page, int size) {
			//beneficiarioProxy.set#columnsGrid(beneficiario.get#columnsGrid());
			return (List<Beneficiario>) sessionFactory.getCurrentSession()
					...
					.add(	
						...
						Restrictions.like("fecha_nacimiento", "%" + query +"%"),Restrictions.like("curp", "%" + query +"%")),Restrictions.like("nombre", "%" + query +"%")),Restrictions.like("apellido_materno", "%" + query +"%")),Restrictions.like("parentesco", "%" + query +"%")),Restrictions.like("apellido_paterno", "%" + query +"%"))	
